...
function requestDebitToken(paymentId = "", bankCode = "", nationalId = "", phone = "") {
    if (paymentId === "") {
    ...
    //1. BANKS - GET array with banks data
    const url = `${baseUrl}/payments/debit/tokenRequest`;
    const body = {
        paymentId: paymentId,
        bankCode: bankCode,
...
function canOpenDebitTokenModal() {
    ...
    if (bankCode === "none" || bankCode === "" || nationalIdType === "none" || nationalIdType === "" || nationalIdNumber === "" || phoneExt === "" || phoneNumber === "") {
...
function populateBankDropdown(dropdownIds) {
    let dropdowns = [];
    for (const dropdownId of dropdownIds) {
        const dropdown = document.getElementById(dropdownId);
        if (!dropdown || dropdown.dataset.loaded === "true") continue; // Evita recargar
    ...
    getBankList()
        .then((banks) => {
            for (let dropdown of dropdowns) {
                // Limpiar cualquier opción anterior (opcional)
                dropdown.innerHTML = '<option value="none" disabled selected hidden>Seleccione un banco</option>';
                ...
                dropdown.dataset.loaded = "true"; // Marcar como ya cargado
                dropdown.disabled = false; // Habilitar los dropdowns después de cargar
            }
        })
        .catch((error) => {
...
document.addEventListener("DOMContentLoaded", async () => {
    await populateBankDropdown(["c2p-bank", "debit-bank"]);
});
...
document.getElementById("debit-continue-button").addEventListener("click", async (event) => {
    ...
    onDebitTokenRequestEventListener(event);

    return;
});
document.getElementById("debit-token-modal-resend-token-button").addEventListener("click", async (event) => {
    onDebitTokenRequestEventListener(event);
    return;
});

document.getElementById("debit-token-modal-close-button").addEventListener("click", async (event) => {
        ...
        if (user.user_data_loaded.custom_fields) {
            ...
            const getInfo = async (employeeID) => {
                ...
                if (loans_IDs != null) {
                    ...
                    payments.sort(function (a, b) { return new Date(a.fields.deadline) - new Date(b.fields.deadline); });
                    ...
                    for (let indexRowPayments = 0; indexRowPayments < payments.length; indexRowPayments++) {
                        ...
                        cell3.innerHTML = `$${roundTwo(payments[indexRowPayments].fields.amount_total)}`;
                        cell3.className = "table_cell_5rows amount";
                        var cell4 = newRow.insertCell(3);
                        //cell4.innerHTML = payments[indexRowPayments].fields.status;
                        if (payments[indexRowPayments].fields.status == "Scheduled" && dataAirtable.fields.payment_type == "Personal" && payments[indexRowPayments].fields.cobranza == "Cobrar") {
                            cell4.innerHTML = "¡Clic para Pagar!";
                        };
                        if (payments[indexRowPayments].fields.status == "Scheduled" && dataAirtable.fields.payment_type == "Personal" && payments[indexRowPayments].fields.cobranza != "Cobrar") {
                            cell4.innerHTML = "Clic para adelantar pago";
                        };
                        if (payments[indexRowPayments].fields.status == "Scheduled" && dataAirtable.fields.payment_type != "Personal") {
                        ...
                        if (payments[indexRowPayments].fields.status == "Delayed" && dataAirtable.fields.payment_type != "Nomina") {
                            cell4.innerHTML = "Atrasado - PAGAR aquí";
                        };
                        if (payments[indexRowPayments].fields.status == "Delayed" && dataAirtable.fields.payment_type == "Nomina") {
                            cell4.innerHTML = "Atrasado";
                        };
                        if (payments[indexRowPayments].fields.status == "Por revisar" || payments[indexRowPayments].fields.status == "Error en datos" || payments[indexRowPayments].fields.status == "No coincide" || payments[indexRowPayments].fields.status == "Error en monto") {
                        ...
                        if (payments[indexRowPayments].fields.status == "Scheduled" && payments[indexRowPayments].fields.cobranza != "Cobrar") {
                            cell5.className = "table_cell_status_pending";
                        };
                        if (payments[indexRowPayments].fields.status == "Scheduled" && payments[indexRowPayments].fields.cobranza == "Cobrar") {
                            cell5.className = "table_cell_status_delayed";
                        };
                        if (payments[indexRowPayments].fields.status == "Paid" || payments[indexRowPayments].fields.status == "Paid late" || payments[indexRowPayments].fields.status == "Refinanciado" || payments[indexRowPayments].fields.status == "Canceled") {
                    ...
                    //v.24 FUNCIÓN PARA PRE-LLENAR DATOS DE PAGO:
                    let whatsapp = dataAirtable.fields.whatsapp;
                    //console.log("whatsapp",whatsapp);
                    // Asegúrate de que el número tenga al menos 5 caracteres (para evitar errores)
                    if (whatsapp.length > 4 && whatsapp.startsWith("58")) {
                        // Parte 1: cambiar "58" por "0" y tomar los siguientes 3 dígitos
                        let extension = "0" + whatsapp.slice(2, 5);
                        ...
                        $(`#celular-6`).html(whatsapp_number);
                    };
                    let cedula = dataAirtable.fields.id_number;
                    //console.log("cedula",cedula);
                    // Asegúrate de que hay un guión para separar letra y número
                    if (cedula.includes("-")) {
                        // Separar por el guión
                        let [letra, numerosConPuntos] = cedula.split("-");
                        //console.log("letra",letra);
                        // Eliminar los puntos del número
                        let numeros = numerosConPuntos.replace(/\./g, "");
                        //console.log("numeros",numeros);
                        $(`#debit-id-letter`).html(letra);
                        $(`#debit-id-number`).html(numeros);
                        $(`#c2p-nid-type`).html(letra);
                        $(`#c2p-nid-number`).html(numeros);
                        $(`#cedula-5`).html(letra);
                        $(`#cedula-6`).html(numeros);
                    };
                    ...
                    //    affidavit_text = document.getElementById("affidavit_text").value + "";
                    ...
                    //        document.getElementById("affidavit_error").style.display = "none";
                    //    } else {
                    //        document.getElementById("affidavit").value = "";
                    //        document.getElementById("affidavit_error").style.display = "block";
                    //    }
                    //};
                    ...
                    table.on("click", "tr", async function () {
                        ...
                        if (filteredPayment[0].fields.status == "Scheduled" || filteredPayment[0].fields.status == "Delayed") {
                            document.getElementById("section-cobranza").style.display = "flex";
                        } else {
                            document.getElementById("section-cobranza").style.display = "none";
                        }
                        switch (filteredPayment[0].fields.payment_type[0]) {
                            case "Nomina":
                                document.getElementById("warning-pay-nomina").style.display = "flex";
                                document.getElementById("form-pay-personal-titulo").style.display = "none";
                                document.getElementById("form-pay-personal").style.display = "none";
                                document.getElementById("warning-pay-domiciliado").style.display = "none";
                                break;
                            case "Personal":
                                document.getElementById("warning-pay-nomina").style.display = "none";
                                document.getElementById("form-pay-personal-titulo").style.display = "flex";
                                document.getElementById("form-pay-personal").style.display = "flex";
                                document.getElementById("warning-pay-domiciliado").style.display = "none";
                                break;
                            case "Domiciliado":
                                document.getElementById("warning-pay-nomina").style.display = "none";
                        ...
                        document.getElementById("payment-id").value = filteredPayment[0].id;
                        $(`#data-payment-amount`).html(roundTwo(Number(filteredPayment[0].fields.amount)));
                        $(`#data-payment-mora`).html(roundTwo(Number(filteredPayment[0].fields.mora)));
                        $(`#data-payment-total`).html(roundTwo(Number(filteredPayment[0].fields.amount_total)));
                        ...
                        $(`#data-payment-bs`).html(roundTwo(tasaBCV * Number(filteredPayment[0].fields.amount_total)));
                        $(`#data-payment-deadline`).html(filteredPayment[0].fields.date_formated);
                        switch (filteredPayment[0].fields.status) {
                            case "Scheduled":
                                document.getElementById("status-scheduled").style.display = "flex";
                                document.getElementById("status-por-revisar").style.display = "none";
                                document.getElementById("status-delayed").style.display = "none";
                                document.getElementById("status-paid").style.display = "none";
                                document.getElementById("status-paid-late").style.display = "none";
                                document.getElementById("wrapper-tasa-bcv").style.display = "flex";
                                document.getElementById("wrapper-bolivares").style.display = "flex";
                                document.getElementById("wrapper-bolivares2").style.display = "flex";
                                $(`#data-payment-bs2`).html(roundTwo(tasaBCV * Number(filteredPayment[0].fields.amount_total)));
                                document.getElementById("wrapper-deadline").style.display = "flex";
                                document.getElementById("wrapper-date").style.display = "none";
                                document.getElementById("wrapper-metodo").style.display = "none";
                                document.getElementById("wrapper-comprobante").style.display = "none";
                                document.getElementById("wrapper-tasa-bcv-pagado").style.display = "none";
                                document.getElementById("wrapper-bolivares-pagado").style.display = "none";
                                document.getElementById("button-reportar-pago").style.display = "flex";
                                break;
                            case "Por revisar":
                                document.getElementById("status-scheduled").style.display = "none";
                                document.getElementById("status-por-revisar").style.display = "flex";
                                document.getElementById("status-delayed").style.display = "none";
                                document.getElementById("status-paid").style.display = "none";
                                document.getElementById("status-paid-late").style.display = "none";
                                document.getElementById("wrapper-tasa-bcv").style.display = "none";
                                document.getElementById("wrapper-bolivares").style.display = "none";
                                document.getElementById("wrapper-bolivares2").style.display = "none";
                                document.getElementById("wrapper-deadline").style.display = "flex";
                                document.getElementById("wrapper-date").style.display = "flex";
                                $(`#data-payment-date`).html(filteredPayment[0].fields.date_of_payment_formated);
                                document.getElementById("wrapper-metodo").style.display = "flex";
                                document.getElementById("wrapper-comprobante").style.display = "flex";
                                document.getElementById("wrapper-tasa-bcv-pagado").style.display = "flex";
                                document.getElementById("wrapper-bolivares-pagado").style.display = "flex";
                                $(`#data-payment-metodo`).html(filteredPayment[0].fields.payment_method);
                                $(`#data-payment-comprobante`).html(filteredPayment[0].fields.reference_number);
                                $(`#data-payment-tasa-pagado`).html(filteredPayment[0].fields.tasa_bcv);
                                $(`#data-payment-bs-pagado`).html(filteredPayment[0].fields.amount_bolivares);
                                document.getElementById("button-reportar-pago").style.display = "none";
                                break;
                            case "Error en datos":
                                document.getElementById("status-scheduled").style.display = "none";
                                document.getElementById("status-por-revisar").style.display = "flex";
                            ...
                            case "Delayed":
                                document.getElementById("status-scheduled").style.display = "none";
                                document.getElementById("status-por-revisar").style.display = "none";
                                document.getElementById("status-delayed").style.display = "flex";
                                document.getElementById("status-paid").style.display = "none";
                                document.getElementById("status-paid-late").style.display = "none";
                                document.getElementById("wrapper-tasa-bcv").style.display = "flex";
                                document.getElementById("wrapper-bolivares").style.display = "flex";
                            ...
                            case "Paid late":
                                ...
                                document.getElementById("status-delayed").style.display = "none";
                                document.getElementById("status-paid").style.display = "none";
                                document.getElementById("status-paid-late").style.display = "flex";
                                document.getElementById("wrapper-tasa-bcv").style.display = "none";
                                document.getElementById("wrapper-bolivares").style.display = "none";
                                document.getElementById("wrapper-bolivares2").style.display = "flex";
                                document.getElementById("wrapper-deadline").style.display = "flex";
                            ...
                            default:
                                ...
                                document.getElementById("wrapper-bolivares").style.display = "none";
                                document.getElementById("wrapper-bolivares2").style.display = "none";
                                document.getElementById("wrapper-deadline").style.display = "none";
                                document.getElementById("wrapper-date").style.display = "none";
                                document.getElementById("wrapper-metodo").style.display = "none";
                                document.getElementById("wrapper-comprobante").style.display = "none";
                                document.getElementById("wrapper-tasa-bcv-pagado").style.display = "none";
